...
class Mailjet extends PHPMailer
{
	function Mailjet($email, $objet, $message_html, $message_texte)
	{
		...
		if ($GLOBALS['meta']['mailjet_adresse_envoi'] == 'oui' && $GLOBALS['meta']['mailjet_adresse_envoi_email'])
			$this->From = $GLOBALS['meta']['mailjet_adresse_envoi_email'];
		else
			$this->From = (isset($GLOBALS['meta']["email_envoi"]) && $GLOBALS['meta']["email_envoi"]) ? $GLOBALS['meta']["email_envoi"] : $GLOBALS['meta']['email_webmaster'];

		// Si plusieurs emails dans le from, pas de Name !
		if (strpos($this->From, ",") === false)
		{
			if ($GLOBALS['meta']['mailjet_adresse_envoi'] == 'oui' && $GLOBALS['meta']['mailjet_adresse_envoi_nom'])
	...
	function UrlsAbsolues()
	{
		...
		if (preg_match_all(',(<(a|link)[[:space:]]+[^<>]*href=["\']?)([^"\' ><[:space:]]+)([^<>]*>),imsS', $this->Body, $liens, PREG_SET_ORDER))
		{
		...
		if (preg_match_all(',(<(img|script)[[:space:]]+[^<>]*src=["\']?)([^"\' ><[:space:]]+)([^<>]*>),imsS', $this->Body, $liens, PREG_SET_ORDER))
		{
			foreach ($liens as $lien)
			{
				if (strncmp($lien[3], "cid:", 4) !== 0)
			...
			if (!preg_match(",^[a-z0-9]+://,i", $im[1]) && 
				...
				&&
					(file_exists($f=$im[1]) // l'image a ete generee depuis le meme cote que l'envoi
					 || (_DIR_RACINE && file_exists($f = _DIR_RACINE . $im[1])) // l'image a ete generee dans le public et on est dans le prive
					 || (!_DIR_RACINE && strncmp($im[1], "../", 3) == 0 && file_exists($f = substr($im[1], 3))) // l'image a ete generee dans le prive et on est dans le public
				)
				&& !isset($html_images[$src]))
			{
