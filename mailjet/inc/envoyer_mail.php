...
function inc_envoyer_mail($destinataire, $sujet, $corps, $from = "", $headers = "")
{
	...
	if (empty($from) && empty($mailjet->From))
	{
		$from = $GLOBALS['meta']["email_envoi"];
		if (empty($from) || !email_valide($from))
		{
	...
	if (count($pieces_jointes))
	{
		foreach ($pieces_jointes as $piece)
		{
			$mailjet->AddAttachment(
				...
				(isset($piece['encodage']) && in_array($piece['encodage'], array('base64', '7bit', '8bit', 'binary', 'quoted-printable'))) ? $piece['encodage'] : 'base64',
