...
{
	$erreurs = array();
	if ($email = _request('mailjet_adresse_envoi_email') && !email_valide($email))
	{
	...
	if (_request('mailjet_smtp') == 'oui')
	{
		...
		else
		{
			...
			{
				if (!preg_match(';^([^.\s/?:]+[.]){0,2}[^.\s/?:]+$;', $h) || gethostbyname($h) == $h)
		...
		elseif(!preg_match(';^[0-9]+$;',$p) || !intval($p))
	...
	if ($emailcc = _request('mailjet_cc') && !email_valide($emailcc))
		$erreurs['mailjet_cc'] = _T('form_email_non_valide');

	if ($emailbcc = _request('mailjet_bcc') && !email_valide($emailbcc))
...
function formulaires_configurer_mailjet_traiter_dist()
{
	...
	if (_request('tester'))
	{
		...
		if ((mailjet_envoyer_mail_test($destinataire, _T('mailjet:corps_email_de_test'))) === true) // OK
